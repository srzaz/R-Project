...
mystats$DiffHigh = abs(mystats$`Last price` - mystats$`Max Price`)
mystats$DiffLow = abs(mystats$`Last price` - mystats$`Min Price`)
mystats$DiffMA = abs(mystats$`Last price` - mystats$MA200D)


##### 5 stocks highest HPR
HPRtable <- mystats[order(mystats$HPR, decreasing = TRUE), ]  # Order data descending
HPRtable <- head(HPRtable[,c(1,2,3,9)],5)



##### Make a table that includes five stocks that are close to five-year low price,
#five stocks that are close to five-year high price, 
#and five stocks that are close to 200 day moving average prices.   
fivelow <- mystats[order(mystats$DiffLow, decreasing = FALSE), ]  
fivelow <- head(fivelow[,c(1,2,3,6,8,20)],5)

fivehigh <- mystats[order(mystats$DiffHigh, decreasing = FALSE), ]  
fivehigh <- head(fivehigh[,c(1,2,3,7,8,19)],5)

fiveMA <- mystats[order(mystats$DiffMA, decreasing = FALSE), ]  
fiveMA <- head(fiveMA[,c(1,2,3,8,15,21)],5)
